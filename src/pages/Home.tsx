...
function MapClickHandler({
  ...
  mode: "from" | "to" | null;
  onLocationSelect: (lat: number, lng: number, mode: "from" | "to") => void;
}) {
  useMapEvents({
    click(e) {
      if (mode) {
...
export default function Home() {
  ...
  const handleFromChange = (val: string) => {
    ...
    if (val.length > 0) {
      setFromSuggestions(
        CITIES.filter(
          (c) => c.toLowerCase().includes(val.toLowerCase()) && c !== search.to,
        ),
      );
    } else {
      setFromSuggestions([]);
  ...
  const handleToChange = (val: string) => {
    ...
    if (val.length > 0) {
      setToSuggestions(
        CITIES.filter(
          (c) =>
            c.toLowerCase().includes(val.toLowerCase()) && c !== search.from,
        ),
      );
    } else {
      setToSuggestions([]);
  ...
  const handleLocationSelect = async (
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to get location name. Using coordinates.", {
        id: "geocode",
      });
      if (mode === "from") {
  ...
  const filteredRides = React.useMemo(() => {
    return rides.filter((r) => {
      ...
      return (
        (!search.from ||
          r.from.toLowerCase().includes(search.from.toLowerCase()) ||
          search.from === "Selected Location") &&
        (!search.to ||
          r.to.toLowerCase().includes(search.to.toLowerCase()) ||
          search.to === "Selected Location") &&
      ...
      <div className="relative rounded-[2rem] overflow-hidden bg-gray-900 text-white p-10 md:p-24 text-center shadow-2xl mx-4 md:mx-auto max-w-7xl mt-6">
        ...
        <div className="relative z-10 max-w-4xl mx-auto space-y-8 pb-16">
          ...
          <p className="text-lg md:text-xl text-gray-300 font-medium max-w-2xl mx-auto leading-relaxed">
            Experience the future of intercity travel. Connect with verified
            co-travelers, share costs, and reduce your carbon footprint.
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 relative group">
              <LocationAutocomplete
                ...
                icon={
                  <MapPin
                    className={`h-5 w-5 ${mapMode === "from" ? "text-blue-600" : "text-gray-400"} transition-colors`}
                  />
                }
                rightActions={
                  <>
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        setMapMode(mapMode === "from" ? null : "from");
                      }}
                      className={`p-1.5 rounded-md transition-colors ${mapMode === "from" ? "text-white bg-blue-600" : "text-gray-400 hover:text-blue-600 hover:bg-blue-50"}`}
                      title="Select on map"
                    >
            ...
            <div className="flex-1 relative group">
              <LocationAutocomplete
                ...
                icon={
                  <MapPin
                    className={`h-5 w-5 ${mapMode === "to" ? "text-blue-600" : "text-gray-400"} transition-colors`}
                  />
                }
                rightActions={
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      setMapMode(mapMode === "to" ? null : "to");
                    }}
                    className={`p-1.5 rounded-md transition-colors ${mapMode === "to" ? "text-white bg-blue-600" : "text-gray-400 hover:text-blue-600 hover:bg-blue-50"}`}
                    title="Select on map"
                  >
                    <Crosshair className="h-4 w-4" />
                  </button>
            ...
            <Button
              size="lg"
              className="h-14 px-8 text-lg w-full md:w-auto rounded-xl bg-blue-600 hover:bg-blue-700 shadow-md hover:shadow-lg transition-all"
            >
          ...
          {mapMode && (
            <div className="h-[400px] rounded-xl overflow-hidden border border-gray-200 relative animate-in slide-in-from-top-4 duration-300">
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] bg-white/95 backdrop-blur px-6 py-3 rounded-full shadow-lg font-medium text-sm text-blue-700 flex items-center gap-2 pointer-events-none border border-blue-200">
                ...
                {mapMode === "from" ? "Pickup" : "Drop-off"} Location
              </div>
              <Button
                variant="ghost"
                size="icon"
